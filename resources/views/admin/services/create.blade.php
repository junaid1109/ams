      ...
      <div class="card-body" style="padding: 20px;">
        <form method="POST" action="{{ route('admin.services.store') }}" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <textarea name="features" class="form-control @error('features') is-invalid @enderror" rows="4">{{ old('features') }}</textarea>
            @error('features')<span class="invalid-feedback">{{ $message }}</span>@enderror
          </div>

          <div class="form-group">
            <label>Pricing (HTML)</label>
            <textarea name="pricing" class="form-control @error('pricing') is-invalid @enderror" rows="4">{{ old('pricing') }}</textarea>
